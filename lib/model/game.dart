...
class Game {
  ...
  void pauseList(BuildContext context) async {
  ...
  Future<void> sendTogetherList(String code) async {
    await FirebaseFirestore.instance.collection('workTogether').doc(code).set({Game.instance.listname: Game.instance.toJson()});
    // await Constants.realtimeDatabase.child('workTogether/$code').update({Game.instance.listname: Game.instance.toJson()});
  }
  ...
  Future<void> restoreList(BuildContext context, Game game) async {
    ...
    if (game.shared) await Helpers.startTimer(context);
  }
